...
async def get_certificate(message: Message):
    try:
        ...
        cat = FSInputFile(
            path=f"C:/Users/user/PycharmProjects/DvijeniePervih/utils/upload/{(db.select_columns_from_users_table(['fio'], message.from_user.id))[0]}.pdf")
...
async def get_help(message: Message):
    await message.answer(text='На прохождение теста даётся одна попытка, без возможности изменения результатов.'
                              ...
                              '/test - команда, для начала тестирования\n'
